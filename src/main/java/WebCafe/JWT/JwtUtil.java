...
public class JwtUtil {
    ...
    private String serect = "TamThieu";
    ...
    public <T> T extractClamis(String token, Function<Claims,T> claimsResolver){
        final Claims claims = extraceAllClaims(token);
        return claimsResolver.apply(claims);
    }

    public Claims extraceAllClaims(String token){
        return Jwts.parser().setSigningKey(serect).parseClaimsJws(token).getBody();
    }
